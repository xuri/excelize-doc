...
`TableSheet` 는 테이블 또는 피벗 테이블의 워크시트 이름을 지정합니다. 이 설정은 필수입니다.

`TableName` 은 테이블 또는 피벗 테이블의 이름을 지정합니다. 이 설정은 필수입니다.
...
`SelectedItems` 옵션은 슬라이서에서 기본적으로 선택되는 항목을 지정하는 데 사용됩니다. 현재 피벗 테이블의 슬라이서에서만 지원됩니다. 선택된 항목은 피벗 테이블에서 선택된 항목 범위 내에 있어야 합니다. 피벗 테이블을 [`AddPivotTable`](pivot.md#AddPivotTable) 함수를 사용하여 생성하는 경우, 피벗 테이블 생성 시 동일한 필드에 대한 선택 항목 범위도 지정해야 합니다.
