...
`ChartTitle` 데이터 형식의 `Title` 필드를 사용하여 차트 제목의 속성을 설정할 수 있습니다. 설정 가능한 속성은 다음과 같습니다:

매개 변수|유형|설명
---|---|---
Fill      | `Fill`          | 차트 제목의 채우기 색상을 설정합니다. `Fill` 속성은 선택 사항입니다.
Border    | `LineOptions`   | 차트 제목의 테두리를 설정합니다. 설정할 수 있는 속성은 셀 서식에 사용되는 테두리 개체와 동일합니다. `Border` 속성은 선택 사항입니다.
Paragraph | `[]RichTextRun` | 차트 제목 텍스트의 서식 있는 텍스트를 설정합니다. `Paragraph` 속성은 선택 사항이며, `Formula` 속성과 동시에 설정할 수 없습니다.
Font      | `*Font`         | 차트 제목 수식 텍스트의 글꼴 속성을 설정합니다. `Font` 속성은 선택 사항입니다.
Formula   | `string`        | 차트 제목 텍스트의 수식을 설정합니다. 예: Sheet1!$A$1. `Formula` 속성은 선택 사항이며, `Paragraph` 속성과 동시에 설정할 수 없습니다.
OffsetX   | `int`           | 차트 제목의 가로 오프셋을 설정합니다. `OffsetX` 속성은 선택 사항입니다. 기본값은 0 이며, `OffsetX` 값은 0 에서 100 사이의 정수여야 합니다.
OffsetY   | `int`           | 차트 제목의 세로 오프셋을 설정합니다. `OffsetY` 속성은 선택 사항입니다. 기본값은 0 이며, `OffsetY` 값은 0 에서 100 사이의 정수여야 합니다.
Width     | `int`           | 차트 제목의 너비를 설정합니다. `Width` 속성은 선택 사항입니다. 기본값은 0 이며, `Width` 값은 0 에서 100 사이의 정수여야 합니다.
Height    | `int`           | 차트 제목의 높이를 설정합니다. `Height` 속성은 선택 사항입니다. 기본값은 0 이며, `Height` 값은 0 에서 100 사이의 정수여야 합니다.
Overlay   | `bool`          | 차트 제목이 차트 위에 겹쳐 표시될지 여부를 설정합니다. `Overlay` 속성은 선택 사항이며 기본값은 `false` 입니다.
...
`ChartPlotArea` 데이터 형식의 `PlotArea` 필드를 사용하여 차트 플롯 영역의 속성을 설정합니다. 설정 가능한 속성은 다음과 같습니다:
