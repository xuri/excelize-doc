...
AutoFitColWidth 는 텍스트 내용 및 폰트 서식에 맞춰 열 너비를 자동으로 조정하는 기능을 제공합니다. 선택된 범위 내에 숨겨진 열이 포함되어 있고 해당 열에 내용이 들어 있는 경우, 이 함수는 숨겨진 열을 다시 표시 합니다. 참고로, 이 함수는 폰트 서식을 기반으로 텍스트 너비를 근사치로 계산하며, 현재 병합된 셀 은 지원하지 않습니다. 따라서 Office 애플리케이션에서 통합 문서를 열었을 때 실제 열 너비가 다르게 보일 수 있습니다. 대용량 워크시트의 경우 이 작업의 처리 속도가 다소 느릴 수 있으므로, 일반적으로는 전체 작업 과정이 완료되는 시점에 열당 한 번씩만 호출하는 것이 좋습니다. 예를 들어, `Sheet1` 의 `D` 열 너비를 자동으로 조정하려면 다음과 같이 실행합니다:
...
SetSheetDimension 은 주어진 범위 참조로 워크시트의 사용 범위를 설정하거나 제거하는 방법을 제공합니다. 워크시트에서 사용된 셀의 행 및 열 경계를 지정합니다. 범위 참조는 `A1` 참조 스타일 (예: `A1:D5`) 을 사용하여 설정됩니다. 빈 범위 참조를 전달하면 워크시트에서 사용된 범위가 제거됩니다.
...
GetSheetDimension 은 워크시트의 사용 범위를 가져오는 방법을 제공합니다.
