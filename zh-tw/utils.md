...
ZipWriter 定義了一個用於將檔案寫入 ZIP 壓縮包的介面。它提供了在壓縮包中建立新檔案、從檔案系統新增檔案以及在寫入完成後關閉壓縮包的方法。
...
上述示例中的佔位符變數 `x` 可以被任何簡單的字符串替換。實際的佔位符名稱在內部被略過，所以以下所有表達式的效果都是等同的：
...
`BarOnly` - 用於設定是否隱藏儲存格中的值，僅顯示資料橫條。
...
<p align="center"><img width="884" src="./images/condition_format_01.png" alt="透過設定條件式格式高亮區域儲存格中的最大值與最小值"></p>
...
電子錶格文檔中的文本有 3 種色彩：十六進制色彩、索引色彩和主題色彩。這些色彩的優先級為：十六進制色彩優先於主題色彩、主題色彩優先於索引色彩。另外，色彩還支援基於十六進制色彩應用色調值，因此可使用 ThemeColor 函數為首選色彩應用色調，以獲取計算出的實際十六進制色彩值。例如，獲取名為 `Sheet1` 的工作表 `A1` 儲存格的富文本中的文字色彩：
...
## 設定 ZIP 寫入器 {#SetZipWriter}

```go
func (f *File) SetZipWriter(fn func(io.Writer) ZipWriter) *File
```

SetZipWriter 設定用於儲存活頁簿的用户自訂 ZIP 寫入函式。
