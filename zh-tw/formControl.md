...
AddFormControl 透過給定的工作表名稱和表單控制項選項在工作表中添加表單控制項。支援的表單控制項類型為：按鈕、核取方塊、群組方塊、標籤、選項按鈕、捲軸和微調按鈕。若需要為表單控制項指定巨集，保存的活頁簿擴展名應為 `.xlsm` 或者 `.xltm`。滾動值應介於 0 到 30000 之間。請注意，如果為核取方塊表單控件設定了存儲格連結，則在選中核取方塊時，Excelize 不會為連結的存儲格分配值。要反映核取方塊狀態，請使用 [`SetCellValue`](cell.md#SetCellValue) 函數將連結儲存格的值設定為 `true`。
