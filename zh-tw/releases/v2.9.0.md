...
* 資料類型 `GraphicOptions` 中添加了新的欄位 `AutoFitIgnoreAspect` 支援添加圖片時以略過比例的方式填滿儲存格
...
* 獲取資料驗證函式 `GetDataValidations` 支援獲取存儲於擴展列表中的資料驗證規則，相關 issue [#1835](https://github.com/xuri/excelize/issues/1835)
...
* 設定超連結函式 `SetCellHyperLink` 支援指定 linkType 為 `None` 以刪除儲存格超連結，相關 issue [#1940](https://github.com/xuri/excelize/issues/1940)
* 添加圖表函式 `AddChart` 支援創建帶有多個相同圖表類型的組合圖表，相關 issue [#1940](https://github.com/xuri/excelize/issues/1940)
* 添加圖表函式 `AddChart` 支援設定散點圖線型
* 使用 `AddPivotTable` 函式創建樞紐分析表時，若資料源引用的儲存格範圍內首行儲存格的值為空，將傳回錯誤，相關 issue [#1945](https://github.com/xuri/excelize/issues/1945)
...
* 修復 v2.8.1 中引入的問題，修復了複製行時，如果工作表中帶有多重儲存格引用的條件式格式或資料驗證，將傳回錯誤的問題
* 修復 v2.8.1 中引入的問題，修復了多次存儲時儲存格的值設定有誤的問題，解決 issue [#1906](https://github.com/xuri/excelize/issues/1906)
...
* 修復部分情況下 `RemoveCol` 函式傳回錯誤的問題，解決 issue [#1829](https://github.com/xuri/excelize/issues/1829)
...
* 存儲活頁簿時將對內部部件排序，以獲得一致的檔案哈希值和正確的互聯網媒體類型，解決 issue [#1889](https://github.com/xuri/excelize/issues/1889)
...
* 修復部分情況下獲取樞紐分析表函式 `GetPivotTables` 傳回樞紐分析表的資料源範圍有誤的問題，解決 issue [#1937](https://github.com/xuri/excelize/issues/1937)
* 修復獲取樣式函式 `GetStyle` 在讀取不包含 `sysClr` 屬性的活頁簿時出現 panic 的問題，解決 issue [#1963](https://github.com/xuri/excelize/issues/1963)
* 修復獲取儲存格富文本函式 `GetCellRichText` 在讀取不包含共享字符串表索引儲存格時傳回錯誤的問題，解決 issue [#1999](https://github.com/xuri/excelize/issues/1999)
...
* 修復因刪除列欄而調整圖片、圖表、圖形等繪圖對象位置後，與圖形相關聯的巨集丟失的問題，解決 issue [#1957](https://github.com/xuri/excelize/issues/1957)
