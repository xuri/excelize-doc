...
* 添加圖表時支持設定不顯示主要橫縱坐標軸
* 通過 [`AddPivotTable`](https://pkg.go.dev/github.com/xuri/excelize/v2@v2.4.1#File.AddPivotTable) 創建樞紐分析表支持通過自定義名稱動態引用數據源
...
* 增加內部共享字符表計數
* 解除通過給定的時間設定存儲格的值時，需要協調世界時 (UTC) 的限制，相關 issue [#409](https://github.com/xuri/excelize/issues/409)
* 增加對內部 XML 控制字符的兼容
...
* 通過 [`Save`](https://pkg.go.dev/github.com/xuri/excelize/v2@v2.4.1#File.Save) 儲存或 [`SaveAs`](https://pkg.go.dev/github.com/xuri/excelize/v2@v2.4.1#File.SaveAs) 另存檔案時的內存使用量相較於上一版本降低最高約 19%
