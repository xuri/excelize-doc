...
* 將資料類型 `HeaderFooterOptions` 中的欄位 `AlignWithMargins` 和 `ScaleWithDoc` 修改為指針類型，解決 issue [#1645](https://github.com/xuri/excelize/issues/1645)
...
* 新增函式 `GetHeaderFooter` 支援獲取工作表頁首頁尾控制字符設定
* 新增函式 `AddSlicer` 支援為表格和樞紐分析表添加交叉分析篩選器
* 新增函式 `GetPivotTables` 支援獲取樞紐分析表
* 新增函式 `DeletePivotTable` 支援刪除樞紐分析表
* 資料類型 `PivotTableOptions` 中添加了新的欄位 `Name` 以支援創建樞紐分析表時指定自訂樞紐分析表名稱
* 新增 7 項公式函式：DBCS、SEARCH、SEARCHB、TEXT、TEXTAFTER、TEXTBEFORE 和 xlfn.ANCHORARRAY
* 新增 `ChartLineType` 列舉類型，以支援設定圖表區域外框線型，相關 issue [#1706](https://github.com/xuri/excelize/issues/1706)
* 資料類型 `Chart` 中添加了新的欄位 `Border` 以支援設定圖表區域外框類型
* 資料類型 `ChartLine` 中添加了新的欄位 `Type` 以支援設定圖表線型
...
* 支援為註解框設定自訂寬度和高度，相關 issue [#1688](https://github.com/xuri/excelize/issues/1688)
...
* 當給定的列高度無效時，函式 `SetRowHeight` 將傳回錯誤提示信息
* 添加圖表函式 `AddChart` 支援設定圖表中的資料標籤位置，相關 issue [#1704](https://github.com/xuri/excelize/issues/1704)
* 添加圖表函式 `AddChart` 支援設定圖表區域、繪圖區域和資料標記的填滿色和透明填滿，在 `Chart`、`ChartPlotArea` 和 `ChartMarker` 數據類型中添加了 `Fill` 選項，相關 issue [#1786](https://github.com/xuri/excelize/issues/1786)
* 添加圖表函式 `AddChart` 支援設定圖表坐標軸文字字型、字號和刪除線格式，相關 issue [#1809](https://github.com/xuri/excelize/issues/1809)
* 資料類型 `ChartSeries` 中新增了 `DataLabelPosition` 欄位，用於指定圖表中各資料系列資料標籤的位置
* 資料類型 `Chart` 中新增了 `BubbleSize` 欄位，用於設定泡泡圖和三維泡泡圖的泡泡大小
...
* 儲存格值讀取函式支援繼承使用 `OpenReader` 函式開啓文檔時所指定的 `Options` 選項設定，相關 issue [#1815](https://github.com/xuri/excelize/issues/1815)
...
* 存儲活頁簿時對內部部件進行排序，以確保在不改變活頁簿內容時生成的檔案具有一致的哈希值，相關 issue [#1732](https://github.com/xuri/excelize/issues/1732)
...
* 修復 v2.8.0 中引入的問題，透過創建特殊自訂名稱的方式設定列印區域和列印標題時不再傳回錯誤
...
* 修復獲取富文本儲存格函式 `GetCellRichText` 在獲取內聯富文本時傳回錯誤的問題
...
* 修復了獲取樣式函式 `GetStyle` 或獲取條件式格式樣式函式 `GetConditionalStyle` 所傳回的小數位數 `DecimalPlaces` 欄位值有誤問題，解決 issue [#1777](https://github.com/xuri/excelize/issues/1777)
