...
* 添加圖片、圖表、圖形和表單控制項等繪圖對象時，偏移設定將不再影響繪圖對象的尺寸，解決 issue [#2001](https://github.com/xuri/excelize/issues/2001)
...
* 新增 `AddHeaderFooterImage` 函式，支援在頁首和頁尾中設定圖片，相關 issue [#1395](https://github.com/xuri/excelize/issues/1395)
...
* 函式 `AddFormControl` 函式支援為復選框類型的表單控件設定存儲格超鏈接，相關 issue [#2113](https://github.com/xuri/excelize/issues/2113)
...
* 函式 `SetPageLayout` 增加了對選項參數的檢查，如果給定的選項值無效，將傳回錯誤提示信息
...
* 修復部分情況下生成重復的「無填滿」格式問題，解決 issue [#2014](https://github.com/xuri/excelize/issues/2014)
...
* 修復部分情況下設定圖表標題字型時出現的 panic 問題，解決 issue [#2102](https://github.com/xuri/excelize/issues/2102)
...
* 修復當部分情況下重命名工作表時，自訂名稱中的工作表名稱更新有誤的問題，解決 issue [#2126](https://github.com/xuri/excelize/issues/2126)
