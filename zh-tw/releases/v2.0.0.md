...
|函式名|增加 `error` 傳回值|列號計算方法變更<sup>\*</sup>|刪除|新增|
|---|---|---|---|---|
...
* 修復透過獲取註解函式 `GetComment`，從包含多個工作表的活頁簿獲取註解時，傳回了錯誤的工作表名稱的問題，解決了相關 issue [#345](https://github.com/xuri/excelize/issues/345)
* 修復獲取工作表全部儲存格函式 `GetRows` 傳回多餘的欄的問題
* 修復部分情況下刪除包含公式的工作表引起文檔損壞的問題

### 性能提升

* 儲存格賦值系列函式、添加註解、圖片、圖表、超連結和圖形函式的性能，均獲得了大幅提升
