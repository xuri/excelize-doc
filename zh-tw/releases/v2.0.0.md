...
* 新增 `DuplicateRowTo` 方法, 支持根據給定的工作表名稱和列號，在指定列後複製該列
* 工作表頁面配置設定函式 `SetPageLayout` 新增對頁面配置方向設定的支持，解決了相關 issue [#318](https://github.com/xuri/excelize/issues/318)
* 工作表頁面配置設定函式 `SetPageLayout` 新增對頁面紙張大小設定的支持
