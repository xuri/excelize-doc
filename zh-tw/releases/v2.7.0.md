...
* 當給定工作表名稱中存在無效字符時將傳回錯誤，不再自動略過無效字符
* `GetCellStyle` 函式不再傳回合併儲存格區域左上角儲存格的樣式
...
* 當 XML 反序列化異常時將傳回錯誤以代替輸出日誌，函式 `GetComments`, `GetDefaultFont` 和 `SetDefaultFont` 增加了 error 類型的錯誤傳回值
...
* `Save`、`Write` 和 `WriteTo` 函式支援指定存儲選項，相關 issue [#744](https://github.com/xuri/excelize/issues/744)
...
* 設定樣式時，當給定的樣式 ID 不存在時將傳回錯誤，相關 issue [#1323](https://github.com/xuri/excelize/issues/1323)
* 流式按列賦值時，若列號未遞增將傳回錯誤，以避免生成的活頁簿損壞，相關 issue [#1139](https://github.com/xuri/excelize/issues/1139)
...
* 存儲活頁簿時將跳過工作表中的不帶有樣式和屬性的空白列，以減少生成檔案的體積，相關 issue [#1383](https://github.com/xuri/excelize/issues/1383)
