...
* 使用 `AddChart` 函數添加圖表時，支持設定自定義折線圖線條色彩，相關 issue [#1345](https://github.com/xuri/excelize/issues/1345)
* 使用 `AddChart` 函數添加圖表時，支持設定自定義坐標軸字型樣式，相關 issue [#320](https://github.com/xuri/excelize/issues/320)
* 添加圖表函數 `AddChart` 支持創建立體折線圖
* 以下函數新增併發安全支持：`SetColWidth`、`GetColWidth`、`SetColVisible`、`GetColVisible`、`SetColStyle` 與 `GetColStyle`
