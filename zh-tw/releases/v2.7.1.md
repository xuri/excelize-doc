...
* 計算儲存格的值函式 `CalcCellValue` 函式對於存在錯誤的公式將在計算結果中傳回公式錯誤代碼，並將詳細錯誤信息在 error 數據類型的傳回值中傳回，相關 issue [#1490](https://github.com/xuri/excelize/issues/1490)
...
* 獲取超連結 `GetCellHyperLink` 函式支援獲取合併儲存格中的超連結
...
* 修復了開啓列高或欄寬為 0 的工作表，存儲後列高欄寬設定失效的問題，解決 issue [#1461](https://github.com/xuri/excelize/issues/1461)
