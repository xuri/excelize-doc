...
通過給定的樣式格式結構體創建樣式並返回樣式索引，樣式索引對於每個活頁簿是唯一的，並且僅對當前活頁簿有效，不可跨活頁簿使用。此功能是併發安全的。請注意，`Font.Color` 需要使用 `RRGGBB` 格式的 RGB 十六進制色值代碼。
...
值|描述
---|---
0 | 根據內容，通過存儲格文本中的第一個非空白字符來確定最終的閱讀順序文字方向：如果它是一個從右到左的強字符，則閱讀順序是從右到左；否則，閱讀順序為從左到右
