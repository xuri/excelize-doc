...
AutoFitColWidth fournit une fonction permettant d'ajuster automatiquement la largeur des colonnes en fonction de leur contenu textuel et de leur format de police. Si la plage sélectionnée contient des colonnes masquées et que celles-ci comportent du contenu, cette fonction les affichera. Notez que cette fonction calcule la largeur du texte de manière approximative, en se basant sur le format de police ; elle ne prend pas encore en charge les cellules fusionnées, et la largeur réelle peut s'avérer différente lors de l'ouverture du classeur dans les applications Office. Ce processus pouvant être relativement lent sur des feuilles de calcul volumineuses, il est recommandé de n'appeler cette fonction qu'une seule fois par colonne, idéalement à la fin de votre traitement. Par exemple, pour ajuster automatiquement la largeur de la colonne `D` sur la feuille `Feuil1`:
