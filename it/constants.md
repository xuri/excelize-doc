...
CultureName è il tipo di codici paese delle lingue supportate per il formato del numero di richiesta.

```go
type CultureName byte
```

Questa sezione definisce l'enumerazione dei tipi di codice paese attualmente supportati per il formato del numero di applicazione.
...
ChartType è il tipo di tipi di grafico supportati.

```go
type ChartType byte
```

Questa sezione definisce l'enumerazione dei tipi di grafico attualmente supportati:
...
Relazione di origine e spazio dei nomi:
...
const (
    ...
    // Le seguenti costanti definiscono l'elemento figlio extLst
    // ([ISO/IEC29500-1:2016] sezione 18.2.10) della cartella di lavoro e del
    // foglio di lavoro elementi estesi con l'aggiunta di nuovi elementi ext figli.
...
Definire la dimensione predefinita della cella e l'unità di misura EMU (unità metriche inglesi):
...
Questa sezione definisce i tipi di convalida dei dati.
...
Questa sezione definisce gli operatori di convalida dei dati.
...
CellType è il tipo di tipo valore della cella.
...
ColorMappingType è il tipo di trasformazione del colore.
