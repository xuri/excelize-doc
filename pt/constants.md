...
CultureName é o tipo de código de país de idioma compatível para aplicar o formato de número.

```go
type CultureName byte
```

Esta seção define a enumeração de tipos de código de país atualmente suportados para o formato de número aplicado.
...
ChartType é o tipo de tipos de gráficos suportados.

```go
type ChartType byte
```

Esta seção define a enumeração dos tipos de gráficos atualmente suportados:
...
Relacionamento de origem e namespace:
...
const (
    ...
    // As constantes a seguir definiram o elemento filho extLst (
    // [ISO/IEC29500-1:2016] seção 18.2.10) da pasta de trabalho e dos
    // elementos da planilha estendidos pela adição de novos elementos ext
    // filhos.
...
Defina o tamanho da célula padrão e a unidade de medida EMU (Unidades Métricas Inglesas):
...
Esta seção define os tipos de validação de dados.
...
Esta seção define os operadores de validação de dados.
...
CellType é o tipo de valor da célula.
...
ColorMappingType é o tipo de transformação de cores.
