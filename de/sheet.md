...
AutoFitColWidth stellt eine Funktion bereit, um die Spaltenbreite automatisch – unter Berücksichtigung des Schriftformats – an den jeweiligen Textinhalt anzupassen. Falls der ausgewählte Bereich ausgeblendete Spalten enthält, die mit Inhalt gefüllt sind, blendet diese Funktion die betreffenden Spalten wieder ein. Bitte beachten Sie, dass diese Funktion die Textbreite auf der Grundlage des Schriftformats lediglich annähernd berechnet; zusammengeführte Zellen (Merged Cells) werden derzeit nicht unterstützt, weshalb die tatsächliche Breite beim Öffnen der Arbeitsmappe in Office-Anwendungen abweichen kann. Bei großen Arbeitsblättern kann dieser Vorgang verhältnismäßig viel Zeit in Anspruch nehmen; daher sollte die Funktion üblicherweise nur einmal pro Spalte – und zwar am Ende Ihres Verarbeitungsprozesses – aufgerufen werden. Beispiel: Automatische Anpassung der Spaltenbreite für Spalte `D` auf dem Arbeitsblatt `Tabelle1`:
