...
`TableSheet` は、テーブルまたはピボットテーブルのワークシート名を指定します。この設定は必須です。

`TableName` は、テーブルまたはピボットテーブルの名前を指定します。この設定は必須です。
...
`SelectedItems` オプションは、スライサーでデフォルトで選択される項目を指定するために使用されます。現在、このオプションはピボットテーブルのスライサーでのみサポートされています。選択される項目は、ピボットテーブルで選択された項目の範囲内である必要があります。ピボットテーブルが [`AddPivotTable`](pivot.md#AddPivotTable) 関数を使用して作成される場合、ピボットテーブルの作成時に、同じフィールドで選択される項目の範囲も指定する必要があります。
