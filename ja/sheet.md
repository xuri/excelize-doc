...
AutoFitColWidth は、フォント書式を考慮しつつ、テキストの内容に合わせて列幅を自動調整する機能を提供します。選択範囲に非表示の列が含まれており、かつそれらの列に内容が入力されている場合、本機能はそれらの非表示列を表示状態に戻します。なお、本機能によるテキスト幅の算出はフォント書式に基づいて行われるものの、あくまで概算となります。また、現時点では結合セルには対応しておらず、Office アプリケーションでブックを開いた際に実際の列幅が異なる場合があります。大規模なワークシートに対してこの処理を実行すると、比較的時間がかかる可能性があります。そのため、通常は処理の最後に、各列につき一度だけ呼び出すようにしてください。例：`Sheet1` の `D` 列の列幅を自動調整する場合：
