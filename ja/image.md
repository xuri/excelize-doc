...
オプションパラメータ `ScaleX` は、グラフオブジェクトの水平方向のスケールを指定します。`ScaleX` の値は、0より大きい浮動小数点数で、小数点以下2桁の精度でなければなりません。デフォルト値は 1.0 で、100% を表します。

オプションパラメータ `ScaleY` は、グラフオブジェクトの垂直方向のスケールを指定します。`ScaleY` の値は、0より大きい浮動小数点数で、小数点以下2桁の精度でなければなりません。デフォルト値は 1.0 で、100% を表します。
...
指定されたワークシート名とセルの座標に基づいてブックのピクチャを取得し、Excel ドキュメントに埋め込まれているピクチャを `[]byte` 型で返します。この関数は、同時実行セーフをサポートします。たとえば、`Sheet1` という名前のワークシートの `A2`セルにピクチャを取得します。この関数は現在、画像の `Format` プロパティからすべてのプロパティを取得することをサポートしておらず、`ScaleX` プロパティと `ScaleY` プロパティの値は小数点以下 2 桁の精度を持つ 0 より大きい浮動小数点数であることに注意してください。
