...
## ضبط عرض العمود تلقائيًا {#AutoFitColWidth}

```go
func (f *File) AutoFitColWidth(sheet, columns string) error
```

توفر الدالة AutoFitColWidth إمكانية الضبط التلقائي لعرض الأعمدة بما يتناسب مع محتواها النصي، مع مراعاة تنسيق الخط المستخدم. وفي حال احتوى النطاق المحدد على أعمدة مخفية تتضمن بيانات، ستقوم هذه الدالة بإظهار تلك الأعمدة المخفية. يُرجى ملاحظة أن هذه الدالة تُجري حساباً تقريبياً لعرض النص استناداً إلى تنسيق الخط، كما أنها لا تدعم حالياً الخلايا المدمجة؛ وعليه، قد يختلف العرض الفعلي للأعمدة عند فتح المصنف في تطبيقات Office. ونظراً لأن هذه العملية قد تستغرق وقتاً طويلاً نسبياً عند تطبيقها على أوراق عمل كبيرة الحجم، فإنه يُنصح عادةً باستدعاء هذه الدالة مرة واحدة فقط لكل عمود، وذلك في نهاية مراحل المعالجة الخاصة بك. على سبيل المثال، لضبط العرض التلقائي للعمود `D` في الورقة `ورقة1`:

```go
err := f.AutoFitColWidth("ورقة1", "D")
```

ضبط عرض الأعمدة تلقائيًا للأعمدة من `D` إلى `F` في `ورقة1`:

```go
err := f.AutoFitColWidth("ورقة1", "D:F")
```
