...
توفر دالة AutoFitColWidth وظيفةً لضبط عرض الأعمدة تلقائيًا وفقًا لمحتواها النصي وتنسيق الخط. إذا كان النطاق المحدد يحتوي على أعمدة مخفية، وكانت هذه الأعمدة تحتوي على نص، فستُظهر هذه الدالة الأعمدة المخفية. تجدر الإشارة إلى أن هذه الدالة تحسب عرض النص تقريبًا بناءً على تنسيق الخط، ولا تدعم حاليًا الخلايا المدمجة. قد يختلف العرض الفعلي عند فتح المصنف في تطبيقات Office. قد تكون هذه العملية بطيئة نسبيًا في أوراق العمل الكبيرة، لذا يُنصح باستدعائها مرة واحدة فقط لكل عمود، في نهاية المعالجة. على سبيل المثال، لضبط عرض العمود `D` في `ورقة1` تلقائيًا:
