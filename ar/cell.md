...
## احصل على خلايا الارتباط التشعبي {#GetHyperLinkCells}

```go
func (f *File) GetHyperLinkCells(sheet, linkType string) ([]string, error)
```

تُرجع GetHyperLinkCells مراجع الخلايا التي تحتوي على ارتباطات تشعبية في اسم ورقة العمل ونوع الارتباط المحددين. المعامل الاختياري `linkType` يُستخدم لتحديد نوع الارتباط، القيم الاختيارية هي `External` لروابط المواقع الإلكترونية، `Location` للانتقال إلى إحدى الخلايا في هذا المصنف، `None` لعدم وجود روابط. إذا كان `linkType` فارغًا، فسيتم إرجاع جميع الارتباطات التشعبية في ورقة العمل.
...
توفر AddComment طريقة لإضافة تعليقات إلى ورقة عمل باستخدام فهرس ورقة العمل والخلية ومجموعة التنسيقات (مثل المؤلف والنص). يُرجى ملاحظة أن الحد الأقصى لطول المؤلف هو 255، والحد الأقصى لطول النص هو 32512، ولا يمكن أن تحتوي كل خلية إلا على تعليق واحد. سيظهر خطأ عند إضافة تعليق إلى خلية تحتوي على تعليق موجود بالفعل. على سبيل المثال، أضف تعليقًا في `Sheet1!A3`:
...
BAHTTEXT                 | تحوّل رقماً إلى نص باللغة التايلاندية وتضيف اللاحقة "باهت".
...
SORTBY                   | تقوم الدالة SORTBY بفرز محتويات نطاق أو صفيف استناداً إلى القيم في نطاق أو صفيف مقابل
...
UNIQUE                   | إرجاع قائمة من القيم الفريدة في قائمة أو نطاق
