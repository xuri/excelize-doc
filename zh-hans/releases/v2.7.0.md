...
* 修改了以下 9 个函数的签名
  ...
  * 将 `func (f *File) GetSheetVisible(sheet string)` 修改为 `func (f *File) GetSheetVisible(sheet string) (bool, error)`
...
* 支持流式设置行内富文本单元格
* 支持流式插入分页符
* 新增 5 项导出错误信息 `ErrStreamSetPanes`、`ErrSheetNameBlank`、`ErrSheetNameInvalid`、`ErrSheetNameLength` 和 `ErrSheetNameSingleQuote`，以便开发者可根据不同的错误类型进行采取相应处理
