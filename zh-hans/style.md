...
值|描述
---|---
0 | 根据内容，通过单元格文本中的第一个非空白字符来确定最终的阅读顺序文字方向：如果它是一个从右到左的强字符，则阅读顺序是从右到左；否则，阅读顺序为从左到右
1 | 总是从左到右: 阅读顺序在单元格中是从左到右的，例如汉语、英语
2 | 总是从右到左: 阅读顺序在单元格中是从右到左的，例如希伯来语
...
### 自定义数字格式 {#number_format}

下面的表格是 Excelize 中支持的索引 `Style.NumFmt` 参数与自定义数字格式类型对照表：
...
下面的表格是 Excelize 中支持的索引 `Style.NumFmt` 参数与泰国语 `th-th` 版本自定义数字格式类型对照表：
