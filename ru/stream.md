...
## Установить структуру столбцов в потоке {#SetColOutlineLevel}

```go
func (sw *StreamWriter) SetColOutlineLevel(col int, level uint8) error
```

Функция SetColOutlineLevel позволяет установить уровень структуры отдельного столбца для объекта `StreamWriter`. Значение параметра `level` варьируется от 1 до 7. Обратите внимание, что функцию `SetColOutlineLevel` необходимо вызывать перед функцией [`SetRow`](stream.md#SetRow). Например, установите уровень структуры столбца `D` равным 2:
...
## Настройка видимости столбцов в потоке {#SetColVisible}

```go
func (sw *StreamWriter) SetColVisible(minVal, maxVal int, visible bool) error
```

Функция SetColVisible позволяет установить видимость одного или нескольких столбцов для объекта `StreamWriter`. Обратите внимание, что функцию `SetColVisible` необходимо вызывать перед функцией [`SetRow`](stream.md#SetRow). Например, скрыть столбец `D`:

```go
err := sw.SetColVisible(4, 4, false)
```

Скрыть столбцы от `D` до `F` (включительно):
