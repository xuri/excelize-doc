...
Функция AutoFitColWidth позволяет автоматически подгонять ширину столбцов под их текстовое содержимое с учетом формата шрифта. Если выбранный диапазон содержит скрытые столбцы, и эти столбцы содержат контент, эта функция отобразит скрытые столбцы. Следует отметить, что эта функция вычисляет ширину текста приблизительно на основе формата шрифта и в настоящее время не поддерживает объединенные ячейки. Фактическая ширина может отличаться при открытии рабочей книги в приложениях Office. Этот процесс может быть относительно медленным на больших листах, поэтому обычно его следует вызывать только один раз для каждого столбца, в конце обработки. Например, автоматическая подгонка ширины столбца для столбца `D` на `Лист1`:
