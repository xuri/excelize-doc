...
## Автоматическая подгонка ширины столбца {#AutoFitColWidth}

```go
func (f *File) AutoFitColWidth(sheet, columns string) error
```

Функция AutoFitColWidth позволяет автоматически подбирать ширину столбцов в соответствии с их текстовым содержимым и форматированием шрифта. Если выбранный диапазон содержит скрытые столбцы, в которых имеется содержимое, данная функция сделает эти столбцы видимыми. Обратите внимание: функция рассчитывает ширину текста приблизительно, исходя из параметров шрифта, и на данный момент не поддерживает объединенные ячейки; поэтому при открытии рабочей книги в приложениях пакета Office фактическая ширина столбцов может отличаться от рассчитанной. На больших рабочих листах этот процесс может занимать значительное время, поэтому, как правило, данную функцию рекомендуется вызывать лишь один раз для каждого столбца — в самом конце обработки данных. Например, автоматический подбор ширины столбца `D` на листе `Лист1` выполняется следующим образом:

```go
err := f.AutoFitColWidth("Лист1", "D")
```

Автоматическая подгонка ширины столбцов от `D` до `F` в `Лист1`:

```go
err := f.AutoFitColWidth("Лист1", "D:F")
```
