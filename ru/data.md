...
`TableSheet` задает имя листа таблицы или сводной таблицы; этот параметр обязателен.

`TableName` задает имя таблицы или сводной таблицы; этот параметр обязателен.
...
`SelectedItems` используется для указания элементов, выбранных по умолчанию в фильтре. В настоящее время он поддерживается только для фильтров в сводных таблицах. Выбранные элементы должны попадать в диапазон элементов, выбранных в сводной таблице. Если сводная таблица создается с помощью функции [`AddPivotTable`](pivot.md#AddPivotTable), то для того же поля необходимо указать диапазон выбранных элементов при создании сводной таблицы.
