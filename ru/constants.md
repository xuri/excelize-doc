...
CultureName — это тип поддерживаемых типов языковых кодов стран для применения числового формата.

```go
type CultureName byte
```

В этом разделе определяется поддерживаемое в настоящее время перечисление типов кодов стран для применения числового формата.
...
ChartType — тип поддерживаемых типов диаграмм.

```go
type ChartType byte
```

В этом разделе определяется перечисление поддерживаемых в настоящее время типов диаграмм:
...
CellType - это тип типа значения ячейки.
