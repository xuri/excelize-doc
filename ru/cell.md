...
## Получить ячейки с гиперссылками {#GetHyperLinkCells}

```go
func (f *File) GetHyperLinkCells(sheet, linkType string) ([]string, error)
```

GetHyperLinkCells возвращает ссылки на ячейки, содержащие гиперссылки, в указанном имени листа и типе ссылки. Необязательный параметр `linkType` используется для определённого типа ссылки, допустимые значения: `External` для ссылок на веб-сайты, `Location` для перехода к одной из ячеек в этой книге, `None` для отсутствия ссылок. Если `linkType` пуст, будут возвращены все гиперссылки на листе.
...
Метод AddComment позволяет добавлять комментарии на лист по заданному индексу листа, ячейке и формату (например, автору и тексту). Обратите внимание, что максимальная длина имени автора составляет 255 символов, а максимальная длина текста — 32512 символов, и каждая ячейка может содержать только один комментарий. При добавлении комментария к ячейке, в которой уже есть комментарий, возникнет ошибка. Например, добавьте комментарий в ячейку `Sheet1!A3`:
...
СОРТПО                          | Функция СОРТПО, которая сортирует содержимое диапазона или массива на основе значений в соответствующем диапазоне или массиве
...
УНИК                            | Ввозвращает список уникальных значений в списке или диапазоне
