...
NewStyle предоставляет функцию для создания стиля для ячеек с заданными параметрами стиля. Эта функция может быть использована для безопасности параллелизма. Обратите внимание, что в поле `Font.Color` используется цвет RGB, представленный в шестнадцатеричном представлении `RRGGBB`.
